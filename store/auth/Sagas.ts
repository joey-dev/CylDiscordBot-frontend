...
export function* authUserSaga(action: AuthUserSagaAction) {
    ...
    if (response === undefined || response.data.hasOwnProperty('status_code') && response.data.status_code !== 200 && response.data.error && response.data.error_description) {
        console.log('fail');
        yield put(actions.authFail({
            name: response.data.error || "wrong code",
            message: response.data.error_description || "code is invalid, or no code has been given",
        }));
    } else {
